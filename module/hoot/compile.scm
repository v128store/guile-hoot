    ...
    (let ((cps (intmap-select cps body)))
      ...
      (define (compile-values exp)
        (match exp
          ...
          (($ $primcall 'fatan #f (x))
           (error "unimplemented" exp))
          (($ $primcall 'fatan2 #f (x y))
           (error "unimplemented" exp))

          (($ $primcall 'logand #f (x y))
           (error "unimplemented" exp))
          (($ $primcall 'logior #f (x y))
           (error "unimplemented" exp))
          (($ $primcall 'logxor #f (x y))
           (error "unimplemented" exp))
          (($ $primcall 'logsub #f (x y))
           (error "unimplemented" exp))
          (($ $primcall 'rsh (x y))
           (error "unimplemented" exp)
           )
          ...
          (($ $primcall 'string->bignum #f (x))
           (error "unimplemented" exp))
          (($ $primcall 'string->symbol #f (x))
           (error "unimplemented" exp))
          (($ $primcall 'symbol->keyword #f (x))
           (error "unimplemented" exp))
          ...
          ;; the ptr and not reference the object.  Annoyingly there are
          ;; no native multi-byte accesses to i8 arrays.
          ...
          (($ $primcall 'atomic-scm-ref/immediate ('atomic-box . 1) (obj))
           (error "unimplemented" exp))
          (($ $primcall 'atomic-scm-swap!/immediate ('atomic-box . 1) (obj val))
           (error "unimplemented" exp))
      ...
      (define (compile-test op param args)
        (match (vector op param args)
          ...
          ;; specific test.
          (#('heap-object? #f (a))
           `(,(local.get a) (ref.test $heap-object)))
          (#('char? #f (a))
           `(,(local.get a)
             (ref.test i31)
             (if #f 'i32
           ...
           `(,(local.get a)
             ...
             (if #f 'i32
                 (,(local.get a)
                  ...
                  (i32.eq))
                 ((i32.const 0)))))
          (#('pair? #f (a))             `(,(local.get a) (ref.test $pair)))
          (#('struct? #f (a))           `(,(local.get a) (ref.test $struct)))
          (#('symbol? #f (a))           `(,(local.get a) (ref.test $symbol)))
          (#('variable? #f (a))         `(,(local.get a) (ref.test $variable)))
          (#('vector? #f (a))           `(,(local.get a) (ref.test $vector)))
          (#('mutable-vector? #f (a))   `(,(local.get a) (ref.test $mutable-vector)))
          (#('immutable-vector? #f (a)) `(,(local.get a) (ref.test $vector)))
          (#('string? #f (a))           `(,(local.get a) (ref.test $string)))
          (#('heap-number? #f (a))      `(,(local.get a) (ref.test $heap-number)))
          (#('hash-table? #f (a))       `(,(local.get a) (ref.test $hash-table)))
          (#('fluid? #f (a))            `(,(local.get a) (ref.test $fluid)))
          (#('dynamic-state? #f (a))    `(,(local.get a) (ref.test $dynamic-state)))
          (#('keyword? #f (a))          `(,(local.get a) (ref.test $keyword)))
          (#('atomic-box? #f (a))       `(,(local.get a) (ref.test $atomic-box)))
          (#('syntax? #f (a))           `(,(local.get a) (ref.test $syntax)))
          (#('program? #f (a))          `(,(local.get a) (ref.test $proc)))
          (#('bytevector? #f (a))       `(,(local.get a) (ref.test $bytevector)))
          (#('weak-table? #f (a))       `(,(local.get a) (ref.test $weak-table)))
          (#('bitvector? #f (a))        `(,(local.get a) (ref.test $bitvector)))
          (#('port? #f (a))             `(,(local.get a) (ref.test $port)))
          (#('bignum? #f (a))           `(,(local.get a) (ref.test $bignum)))
          (#('flonum? #f (a))           `(,(local.get a) (ref.test $flonum)))
          (#('compnum? #f (a))          `(,(local.get a) (ref.test $compnum)))
          (#('fracnum? #f (a))          `(,(local.get a) (ref.test $fracnum)))
