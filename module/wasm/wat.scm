  ...
  (define (unfold-instruction inst)
    ...
    (match inst
      ...
      (((and tag 'ref.null) id . args)
       `(,@args ,tag ,id))
      (((and tag (or 'table.grow 'table.set 'table.get)) id . args)
       `(,@args ,tag ,id))
      (((and tag (or 'struct.new)) id . args)
       `(,@args ,tag ,id))
      (((and tag (or 'struct.set 'struct.get)) ti fi . args)
       `(,@args ,tag ,ti ,fi))
      (((and tag (or 'ref.test 'ref.cast)) id . args)
       `(,@args ,tag ,id))
      (((and tag 'string.const) str . args)
       `(,@args ,tag ,str))
      (((and tag (or 'array.get 'array.set 'array.get_u 'array.get_s)) ti . args)
       `(,@args ,tag ,ti))
      (((and tag (or 'array.new 'array.new_default)) ti . args)
       `(,@args ,tag ,ti))
      (((and tag 'return_call) (? id-or-idx? id) . args)
       `(,@args ,tag ,id))
      (((and tag 'table.size) id . args)
       `(,@args ,tag ,id))
      ((tag . args)
       `(,@args ,tag))))
