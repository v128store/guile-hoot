...
(define (resolve-wasm mod)
  (define-syntax-rule (define-name-store add-id! resolve-id)
    (define-values (add-id! resolve-id)
      (let ((count 0)
            (ids (make-hash-table)))
        (values (lambda (id)
                  (let ((idx count))
                    (set! count (1+ count))
                    (hashq-set! ids id idx)
                    idx))
                (lambda (id)
                  (cond
                   ((exact-integer? id) id)
  ...
  (define-name-store add-type-id! resolve-type)
  (define-name-store add-func-id! resolve-func)
  (define-name-store add-table-id! resolve-table)
  (define-name-store add-memory-id! resolve-memory)
  (define-name-store add-global-id! resolve-global)
  (define-name-store add-elem-id! resolve-elem)
  (define-name-store add-data-id! resolve-data)
  (define-name-store add-tag-id! resolve-tag)
